...
$id = $_GET[ 'wptofb' ];
$wptofbtemplate = new WpToFbTemplate();
$data = $wptofbtemplate->get_wptofb_data( $id );


//id does not exist
if( empty( $data ) ):
exit( _e( 'No data found', 'wptofacebook' ) );
endif;
...
include_once( WP_PLUGIN_DIR . '/wptofacebook/tpls/header.php' );

if( !$facebook_data[ 'like' ] ){
	//contents for no fans
	include_once( WP_PLUGIN_DIR . '/wptofacebook/tpls/nofans.php' );
}

if ( $facebook_data[ 'like' ] || ( !$facebook_data[ 'like' ] && !$data->hide_for_nofans ) ){
	include_once( WP_PLUGIN_DIR . '/wptofacebook/tpls/introtext.php' );
	include_once( WP_PLUGIN_DIR . '/wptofacebook/tpls/'.$data->tpl.'/tpl.php' );
	include_once( WP_PLUGIN_DIR . '/wptofacebook/tpls/footer.php' );
}else{
	include_once( WP_PLUGIN_DIR . '/wptofacebook/tpls/nofans_footer.php' );
}
