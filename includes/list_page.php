	...
	<form method="post" action="<?php echo admin_url( 'admin.php?page=edit-wptofb' );?>">
		<div class="submit">
			<input type="submit" name="new_wptofb" 
				value="<?php _e( 'New Facebook App Connection', 'wptofacebook' ); ?>" />
		</div>
<?php 
//delete record
...
if( $num_records > 0 ){
	?>
	<h3><?php _e( 'List of Facebook Pages Connections', 'wptofacebook' );?></h3>
	<table class="widefat post fixed" cellspacing="0">
		<thead>
			<tr>
				<th class="manage-column column-title" id="wptofb_title" scope="col"><?php _e( 'Title' );?></th>
				<th class="manage-column column-url" id="wptofb_url" scope="col"><?php _e( 'Canvas Page', 'wptofacebook' );?></th>
		...
		<tbody>
			<tr id="rec-<?php echo $record_id;?>" class="<?php echo $pre;?>iedit">
				<td class="column-title"><strong> <a class="row-title"
					title="<?php echo sprintf( __( 'Edit \'%s\'', 'wptofacebook' ), $record_title );?>"
	...
	echo '<p>';
	_e( 'No records found' , 'wptofacebook' );
	echo '</p>';
}
?>
	</form>
